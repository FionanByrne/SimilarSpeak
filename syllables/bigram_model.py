...
def pronouncable(syllable: str, thresh: float, dictionary=None):
    """
    ...
    if len(syllable_split) == 0:  # Emtpy Syllable
        return True
    if all(p in consonants for p in syllable_split):  # No vowel sounds
        return False
    else:
...
print(pronouncable("AH T K L", 0.01))
